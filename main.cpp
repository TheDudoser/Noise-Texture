...
int main()
{
    ...
    sf::Vector3f pos = sf::Vector3f(-8.0f, -8.0f, -8.0f);
    sf::Clock clock;
    int framesStill = 1;

    sf::RenderWindow window(sf::VideoMode(w, h), "3d scene", sf::Style::Titlebar | sf::Style::Close);
    ...
    sf::Shader shader;
    shader.loadFromFile("C:\\Users\\mrego\\CLionProjects\\noise-texture\\OutputShader.frag", sf::Shader::Fragment);
    ...
    while (window.isOpen())
    {
        ...
        if (mouseHidden)
        {
            ...
            float my = ((float)mouseY / h + 0.2f) * mouseSensitivity; // было - 0.2f
